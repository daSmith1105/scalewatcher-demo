...
class TableTest extends React.Component {
                  ...
                  { g.length > 0 ? g.map( event => {
                                                    ...
                                                    // Lpr events
                                                        returnValue = <Brightness1Icon  className="event-icon" 
                                                                                    ...
                                                                                    key={ event.eId}
                                                                                    onMouseEnter={ () => this.setState({ currentEvent: event, showTooltip: true }) }
    ...
    clearTooltip = () => {
        this.setState({ currentEvent: {}, showTooltip: false })
    }

    setCurrentEvent = (event) => {
        this.setState({ currentEvent: event, showTooltip: true })
    }
            ...
            <div style={{ width: '100%' }} >
                  { this.state.showTooltip ? 
                        <div className="speech-bubble" style={{ textAlign: 'left', zIndex: 20,padding: 5, position: 'absolute', top: 100, left: '40%', height: 240, width: 260 }} >
                            <p>id: {this.state.currentEvent.eId}</p>
                            <p>transaction: {this.state.currentEvent.tId}</p> 
                            <p>location: {this.state.currentEvent.location}</p>
                            <p>type: {this.state.currentEvent.type}</p>
                            <p>data: {this.state.currentEvent.data}</p>
                            <p>time: {this.state.currentEvent.timestamp}</p> 
                        </div> :
                        null
                    }
                <ReactTable
