...
parseTooltipDataDisplay = event => {
    ...
    if ( event.type === 'LprRead' ) {
      ...
      let splitData = event.data.split(',');
        formatted = `${splitData[0]} < ${splitData[1]}`;
    ...
    return (
      <Col xs={12} onMouseMove={ this._onMouseMove }>
          { this.state.showTooltip ? 
                    <div className="speech-bubble" style={{ textAlign: 'left', zIndex: 100, position: 'absolute', top: this.state.y - 220,  left: this.state.x + (this.state.x / 20), height: 'auto', width: 'auto' }} >
        ...
        <Row>
          <h1>Scalewatcher</h1>
          <p>Mouse {this.state.x}, {this.state.y}</p>
        </Row>
